...
class WikiAutoWithActionsConfig(datasets.BuilderConfig):
    """Builder config for Wiki-Auto with Actions"""
    ...
    def __init__(self, corpora=_CORPORA, actions=9, **kwargs):
        ...
        super(WikiAutoWithActionsConfig, self).__init__(
...
class WikiAutoWithActions(datasets.GeneratorBasedBuilder):
    """
    ...
    for num in action_nums:
        for k, v in corpora_name_dict.items():
            ...
            BUILDER_CONFIGS.append(WikiAutoWithActionsConfig(name=fname, corpora=v, actions=num))
